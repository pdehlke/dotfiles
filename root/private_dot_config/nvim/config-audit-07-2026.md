---
title: "95 Plugins, 36 Extras: One Neovim Config, Fully Audited"
slug: config-audit-07-2026
summary: >-
  A 95-plugin Neovim + LazyVim setup audited end to end: zero dead config.
description: >-
  Complete July 2026 audit of a Neovim 0.12 + LazyVim v16 configuration:
  core files, keymap architecture, and a documentation cross-reference for
  all 95 plugins. Verdict: no duplications or dead config.
---
...
* Platform: macOS Darwin 25.5.0 (also runs on Raspberry Pi; see lazy.lua
  concurrency handling)
* Formatter: StyLua (4-space indent, 120 col width)
* Source of truth: chezmoi source tree at
  `~/.yadr/root/private_dot_config/nvim/`, applied to `~/.config/nvim/`
...
* **direnv.nvim loads first**, before LazyVim core, so plugins depending on
  ...
  enabled (`󱚟` icon), keybindings `<leader>da/dd/dr/de`
  (allow/deny/reload/edit), silent autoload notifications. A `DirenvLoaded`
  -> `DirChanged` autocmd re-checks direnv once after a directory change.
...
* `Home` / `End` line start/end (all modes; kitty maps ⌥↑/⌥↓ to
  End/Home)
