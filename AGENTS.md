...
- To add an encrypted file: `chezmoi-add ~/.secret-file` (the `chezmoi-add` and `pchezmoi` helpers live in this repo under `root/dot_local/bin/` and operate on the private repo)
- `~/.Codex` and `~/.agents` are managed entirely by `.yadr-private` (not this repo) -- there is no `dot_claude/` or `dot_agents/` here. Plain `chezmoi apply` (public layer only) does not touch them; only `dotfiles-apply` does.
