...
module Vundle
  ...
  def self.add_plugin_to_vundle(plugin_repo)
    ...
    last_bundle_dir = vundles.rindex{ |line| line =~ /^Plug / }
    last_bundle_dir = last_bundle_dir ? last_bundle_dir+1 : 0
    vundles.insert last_bundle_dir, "Plug '#{plugin_repo}'"
    write_vundles_to_file vundles
  end

  def self.remove_plugin_from_vundle(plugin_repo)
    vundles = vundles_from_file
    deleted_value = vundles.reject!{ |line| line =~ /Plug '#{plugin_repo}'/ }
  ...
  def self.update_vundle
    system "vim --noplugin -u #{ENV['HOME']}/.vimrc -N \"+set hidden\" \"+syntax on\" \"+let g:session_autosave = 'no'\" +PlugClean +PlugInstall! +qall"
  end
